...
# events_path[1:3] |>
#   map(possibly(\(path1) vroom(path1,delim = ",",col_names = T,show_col_types = F,col_types = c(`Col 9` = "c"),col_select = c(Date,Time,Type,`Col 9`)),tibble()))

gluc_path[1:3] |>
  map(possibly(\(path2) vroom(path2,delim = ",",col_names = T,show_col_types = F,col_types = c(Type = "c"),col_select = c(`Unique Record ID`,Date,Time,Type,Gl)),tibble()))



map2(
  gluc_path[1:3] |>
    map(possibly(\(path2) vroom(path2,delim = ",",col_names = T,show_col_types = F,col_types = c(Type = "c"),col_select = c(`Unique Record ID`,Date,Time,Type,Gl),n_max = 2),tibble())),
  gluc_path[1:3] |>
    map(possibly(\(path2) data.table::fread(path2,select = c(1:5),col.names = c("Unique Record ID","Date","Time","Type","Gl"),colClasses = c("V2" = "Date","V4" = "character")),tibble())),
  bind_rows) |>
...
my_fun <- function(events, gluc, index = NULL, ...) {
  ...
  list1 <- map2(
    events |>
      map(possibly(\(path) vroom(path,delim = ",",col_names = T,show_col_types = F,col_types = c(`Col 9` = "c"),col_select = c(Date,Time,Type,`Col 9`)),tibble())) |>
    ...
    map2(
      ...
      gluc |>
        map(possibly(\(path) data.table::fread(path,select = c(1:5),col.names = c("Unique Record ID","Date","Time","Type","Gl"),colClasses = c("V2" = "Date","V4" = "character")),tibble())),
      bind_rows) |>
