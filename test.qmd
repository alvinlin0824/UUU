...
df |>
   mutate(# Reference Level
          Level = case_when(round({{ref}} + 0.001) < 100 ~ "<100 mg/dL[5.6 mmol/L]",
                            .default = ">=100 mg/dL[5.6 mmol/L]")) |>
   # Overall Reference Level
   bind_rows(
             df |>
             mutate(Level = "Overall Levels (100 mg/dL breakpoint)")
   ) |>
   ...
   filter((Measure %in% c("Difference", "Absolute Difference") &
          Level %in% c("<100 mg/dL[5.6 mmol/L]", "Overall Levels (100 mg/dL breakpoint)")) |
   ...
   opt_stylize(style = 6, color = "blue")
    ...
    gt_group_var <- group_var[1:length(group_var) - 1]
     ...
     # Add noise
     # mutate(across(contains("Difference"), ~ round(.x + 0.001,1))) |> 
...
Ap |> 
   ...
   diff_measure_fun(ref = Reference, cgm = Gl,group_var = c("Age Group","Reference Type","Level"), unit = "mg/dL",reference_breakpoint = T)
```
<!-- round(c(0.15,0.25)/0.1)*0.1 -->
```{r}
Ap |> 
   select(!c(`Difference(mg/dL)`:`Absolute Relative Difference(%)`)) |> 
   filter(Day <= 15) |> 
   filter(between(Gl,40,400)) |> 
   diff_measure_fun(ref = Reference, cgm = Gl,group_var = c("Age Group","Reference Type","Ref. Glucose Level"), unit = "mg/dL",reference_breakpoint = F) |> View()
```
