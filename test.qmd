...
```{r}
system_agreement_fun <- function(data, ref, cgm, group_var, breakpoint = 70, long = T, wider_name = NULL) {
...
Ap |> 
   filter(between(Gl,40,400)) |>
   system_agreement_fun(wider_name = `Reference Type`,ref = Reference, cgm = Gl, long = T, group_var = c("Age Group","Reference Type"), breakpoint = 70)
```
...
haven::read_sas("adc21206_paired_ca.sas7bdat") |>
   ...
   filter(nday <= 15) |>
   filter(between(cgm_rslt,40,400)) |>
   system_agreement_fun(wider_name = NULL, ref = ref_rslt, cgm = cgm_rslt, long = F, group_var = c("age_grp","ref_type","trt_type"), breakpoint = 70)
```
