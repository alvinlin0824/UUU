...
error_grid_plot <- function (df, ref, cgm, unit = "gram", group_var,
...
Ap |> 
   filter(between(Gl,40,400)) |> 
    group_split(pick(c("Reference Type","Age Group"))) |>
    map(\(df) df |> select(c("Reference Type","Age Group")))
```
...
Ap |> 
   filter(between(Gl,40,400)) |> 
   filter(`Age Group` == "Ages 18+" & `Reference Type` == "BG") |> 
