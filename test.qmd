...
concur_fun <- function(data, bin, ref, cgm, group_var, trans = FALSE){
...
bins <- c(0,40,60,80,120,160,200,250,300,350,400,Inf)
Ap |>
   filter(`Reference Type` == "YSI") |> 
   concur_fun(bin = bins,ref = Reference, cgm = Gl,group_var = c("Age Group"),trans = F) 
```
...
haven::read_sas("adc21206_paired_ca_orr.sas7bdat")) |> 
   filter(nday <= 15) |> 
   filter(ref_type == "YSI") |> 
   concur_fun(bin = bins,ref = ref_rslt, cgm = cgm_rslt,group_var = c("age_grp"),trans = F)
```
