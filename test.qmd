...
system_agreement_fun <- function(data, ref, cgm, unit = "mg/dL",group_var, breakpoint = 70, long = T, wider_name = NULL) {
...
if (long == TRUE) {
 ...
 sys <- data |>
        mutate(Level = case_when(round(Reference + 0.001,1) < breakpoint 
                                 ~ str_c("<",breakpoint_string," mg/dL"),
                                 .default = str_c(">=",breakpoint_string," mg/dL"))) |>
        # Overall
        bind_rows(data |>
...
#| column: page
Ap_mmol |>
        system_agreement_fun(wider_name = NULL,ref = Reference_mmol, cgm = Gl_mmol, unit = "mmol/L",long = F, group_var = c("Sensor","Day"), breakpoint = 80)
```
...
haven::read_sas("adc21206_paired_ca.sas7bdat") |> 
   filter(nday <= 15) |> 
   filter(between(cgm_rslt,40,400)) |> 
   system_agreement_fun(wider_name = ref_type, ref = ref_rslt, cgm = cgm_rslt, long = T, group_var = c("age_grp","ref_type"), breakpoint = 80)
```
...
Ap |> 
   filter(between(Gl,40,400)) |>
   system_agreement_fun(wider_name = `Reference Type`,ref = Reference, cgm = Gl, unit = "mg/dL",long = T, group_var = c("Age Group","Reference Type"), breakpoint = 70)
```
