...
diff_measure_fun <- function(data, ref, cgm, col_subject, group_var, unit = "mg/dL", reference_breakpoint = TRUE) {
...
Ap_mmol |>
   ...
   # filter(Day <= 15) |> 
   # filter(between(Gl,40,400)) |> 
   diff_measure_fun(ref = Reference_mmol, cgm = Gl, col_subject = `Subject ID` ,group_var = NULL, unit = "mmol/L",reference_breakpoint = T)
```
...
haven::read_sas("adc21206_paired_ca.sas7bdat") |> 
   ...
   diff_measure_fun(ref = ref_rslt, cgm = cgm_rslt,col_subject = subjid, group_var = c("age_grp","ref_type"), unit = "mg/dL",reference_breakpoint = F)
```
...
Ap |> 
   ...
   filter(Day <= 15) |> 
   filter(between(Gl,40,400)) |> 
   diff_measure_fun(ref = Reference, cgm = Gl, col_subject = `Subject ID`, group_var = c("Age Group","Reference Type"), unit = "mg/dL",reference_breakpoint = T)
```
